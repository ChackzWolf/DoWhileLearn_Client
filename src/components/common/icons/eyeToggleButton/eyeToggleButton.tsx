...
interface EyeCheckboxProps {
  onClick: () => void;
}

const EyeCheckbox: React.FC<EyeCheckboxProps> = ({ onClick }) => {
  ...
  return (
    <label className="absolute flex items-center cursor-pointer select-none text-gray-500 text-base right-10">
      <input
        type="checkbox"
        checked={isChecked}
      ...
      <svg
        className={`absolute transition-transform duration-500 ease-in-out ${
          isChecked ? 'scale-0 opacity-0' : 'scale-100 opacity-70'
        }`}
      ...
      <svg
        className={`absolute transition-transform duration-500 ease-in-out ${
          isChecked ? 'scale-100 opacity-70' : 'scale-0 opacity-0'
        }`}
        xmlns="http://www.w3.org/2000/svg"
        height="1em"
        viewBox="0 0 640 512"
