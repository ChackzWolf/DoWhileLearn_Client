...
const StudentReviews:React.FC<{ courseId: string | undefined, isPurchased:boolean}> = ({courseId, isPurchased}) => {
  ...
  useEffect(()=> {
     const fetchReviews = async()=> {
        const reviewData = await axios.get(userEndpoint.fetchReviewsOfCourse, {params:{courseId}});
  ...
  useEffect(() => {
    const setupReview = () => {
      const userExists = reviews.some((review: any) => review.userId === userId);
      if (userExists && reviews) {
        console.log('userExists');
  
        const review = reviews.find((review: any) => review.userId === userId);
  
        if (review) { // Check if review is not undefined
          const filteredUsers = reviews.filter((review: any) => review.userId !== userId); // Should filter by !== to remove user's review
      ...
      <div className="max-w-7xl mx-auto p-1">
        ...
        >
          ...
          <div className="lg:col-span-2 ">
            ...
            >
              <h2 className="text-2xl font-bold text-purple-600 mb-6">Student Reviews</h2>
{isPurchased ? 
    ...
    >
      <div className="flex items-center justify-between mb-4">
        <h2 className="text-xl font-semibold text-purple-600">Your feedback</h2>
      </div>
      <div className="mb-4">
      ...
      <div
        className="w-full bg-white border-gray-300 rounded-lg px-3 text-gray-700 focus:outline-none focus:ring-2 focus:ring-purple-500 focus:border-transparent resize-none"
  ...
  >
    <div className="flex items-center justify-between mb-4">
      <h2 className="text-xl font-semibold text-purple-600">Leave a Review</h2>
    </div>
    <div className="mb-4">
     ...
     {isUploadingReview ? <Spinner/> : 'Submit Review'} 
              ...
              >
                <div className="flex items-center justify-between">
                  <div>
                    <div className="text-1xl font-semibold text-purple-600">4.8</div>
                    <div className="flex items-center gap-1 my-2">
                      {renderStars(4.8)}
                    </div>
                  ...
                  <div className="text-right">
                    <div className="text-1xl font-semibold text-gray-800">2,847</div>
...
{ isPurchased ? <motion.div
  ...
  >
    <div className="flex items-center justify-between mb-4">
      <h2 className="text-xl font-semibold text-purple-600">Be the first to Review</h2>
    </div>
    <div className="mb-4">
      <div className="flex items-center gap-1 my-2">
        {renderStarsForRating()}
    ...
    <button
      ...
      className="w-full mt-4 bg-purple-600 hover:bg-purple-700 text-white font-semibold py-2 rounded-lg transition-all duration-300"
    >
{isUploadingReview ? <Spinner/> : 'Submit Review'}     </button>
  </motion.div>:''
        }   

        
        <h1 className="text-2xl font-bold text-gray-600  m-10">No reviews yet.</h1>
      </div>}
      <ToastContainer />
    </motion.div>
  );
